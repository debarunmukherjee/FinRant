...
export default function ViewInvites() {
                                ...
                                <tr key={index}>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <div className="ml-4">
                                                <div className="text-sm font-medium text-gray-900">{listItem.planName}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-gray-900">{listItem.name}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-gray-900">{listItem.email}</div>
                                    </td>
            ...
            <Modal title="Reject Invite" open={openRejectModal} setOpen={setOpenRejectModal} actionText="Reject" onClickAction={sendRejectRequest} isDangerAction={true}>
                <div className="mt-4">
                    <p className="block text-base font-medium">
                        Are you sure you want to reject your invite for <span className="font-bold">{invitesList && invitesList[rejectedInviteId].planName}</span>?
                    </p>
                </div>
            </Modal>
        </div>
    ...
    return (
        <Authenticated
            auth={props.auth}
            errors={props.errors}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Your Invites</h2>}
        >
            <div className="py-12">
